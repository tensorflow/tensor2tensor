...
def GeneralLSTMCell(candidate_transform,
                   memory_transform_fn=None,
                   gate_nonlinearity=core.Sigmoid,
                   candidate_nonlinearity=core.Tanh,
                   sigmoid_bias=0.5):
  ...
  forget_block = [  # u_t
      cb.Dup,
      cb.Parallel(
      cb.Model(gate_nonlinearity(),  core.AddConstant(constant=sigmoid_bias)),
      cb.Model(candidate_nonlinearity(),  core.AddConstant(constant=sigmoid_bias))
      ), 
      cb.Multiply(),
  ]
  input_block = [  # u_t
  ...
  return cb.Model(
      cb.Dup(), cb.Dup(),
      cb.Parallel(memory_transform, cb.Multiply, forget_block, candidate_block),
      cb.Gate(),
  )
