...
def conv2d(name, x, output_channels, filter_size=None, stride=None,
  ...
  with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
    ...
    x = tf.nn.conv2d(x, filters=w, strides=stride_shape, padding="VALID", data_format="NHWC")
...
def merge_level_and_latent_dist(level_dist, latent_dist,
  ...
  if merge_std == "normal":
    ...
    scale = latent_std
  tf.compat.v1.summary.scalar("latent_scale", tf.reduce_mean(scale))
  return tf.compat.v1.distributions.Normal(loc=new_mean, scale=scale)
...
def level_cond_prior(prior_dist, z, latent, hparams, state):
  ...
  if latent_dist_encoder == "pointwise":
    ...
    if latent_skip:
      cond_dist = tf.compat.v1.distributions.Normal(
  ...
  tf.compat.v1.summary.histogram("split_prior_mean", prior_dist.loc)
  tf.compat.v1.summary.histogram("split_prior_scale", prior_dist.scale)
...
def compute_prior(name, z, latent, hparams, condition=False, state=None):
  ...
  with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
    ...
    dist = tf.compat.v1.distributions.Normal(mean, scale)
    return dist, state
...
def scale_gaussian_prior(name, z, logscale_factor=3.0, trainable=True):
  ...
  with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
    ...
    tf.compat.v1.summary.scalar("gaussian_log_scale", tf.reduce_mean(log_scale))
    return tf.compat.v1.distributions.Normal(
