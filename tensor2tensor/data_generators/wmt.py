...
class WMTProblem(problem.Problem):
  ...
  @property
  def dev_generator(self, tmp_dir, is_training):
    return self.train_generator(tmp_dir, is_training)
  ...
  def feature_encoders(self, data_dir):
    ...
    vocab_filename = os.path.join(data_dir, "%s.%d" % (self.vocab_name, self.target_vocab_size))
...
class SETimesMkEnTokens32k(WMTProblem):
  ...
  def train_generator(self, tmp_dir, train):
    ...
    vocab_size = self.targeted_vocab_size
    symbolizer_vocab = generator_utils.get_or_generate_vocab(
        tmp_dir, "tokens.vocab-mk-en.%d" % vocab_size, vocab_size,
